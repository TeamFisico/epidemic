...
int main()
{
    ...
    Simulation prova{800, 1.0, 0.025, 0.15, 0.02, 0.0005, {nS, nE, nI, nR, 0}};
    sf::RenderWindow window(sf::VideoMode(1000, 1000), "My window");
    window.display();
    sf::VertexArray locations(sf::Triangles, 24 * WAYPOINTS_SIZE);
    int count = 0;
    ...
    while(window.isOpen()){
        ...
        if (counter == 1){ prova.move(true); }
        else { prova.move(false);}
        prova.spread();
...
//        }
        ...
        sf::VertexArray people(sf::Quads,POPULATION_SIZE * 4);
        double x_0, y_0;
        r = 1;
        count = 0;
        for(auto& p: prova.People)
        {
                if(!p.is_at_home())
                {
                    if (p.current_status() == Status::Susceptible)
                    {
                    ...
                    else if (p.current_status() == Status::Infected)
                    {
                    ...
                    x_0 = p.get_position().get_X();
                    y_0 = p.get_position().get_Y();
