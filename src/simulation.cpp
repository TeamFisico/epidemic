...
Simulation::Simulation(int S, int E, int I, int R, int Location_number, double people_per_home, double alpha,
    ...
    : alpha{alpha}, beta{beta}, gamma{gamma}, spread_radius{spread_radius}, time_in_days{time_in_days}
{
    ...
    for (int i = 1; i < R; ++i)
    {
        Position rpos = rand_pos(100, 100);
        ; // <- Code to generate a random position
...
std::vector<Person *> Simulation::Close_People(Person &current_person)
{
    ...
    for (unsigned int i = 0; i < Population.size(); ++i)
    {
        ...
        if (pos->InRadius(*current_person.get_pos(), spread_radius) && a->operator[](i).get_condition() == State::S)
        {                                          // check if person is close_enough and Susceptible
