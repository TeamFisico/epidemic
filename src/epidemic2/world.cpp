...
World::World(double Side_length, int number_of_clusters, int number_of_location, int S, int E, int I, int R)
{
    ...
    {
        if (i == number_of_clusters - 1) { loc_num.operator[](i) += loc_left; }
        else
        {
    ...
    for (int i = 0; i < number_of_clusters; ++i)
    {
        if (i == number_of_clusters - 1) { S_pop_num.operator[](i) += S_pop_left; }
        else
        {
    ...
    // an uniform distribution that assign 10 individuals at a time(or the remaiming ones)
    ...
    for (int i = 0; i < number_of_clusters; ++i)
    {
        clusters.emplace_back(S_pop_num.operator[](i), E_pop_num.operator[](i), I_pop_num.operator[](i),
                              R_pop_num.operator[](i), loc_num.operator[](i), cluster_areas.operator[](i));
    }
}
