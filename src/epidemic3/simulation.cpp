...
//families
{
    ...
    for (int i = 0; i < POPULATION_SIZE; i += family_size) //loop over
    {
        ...
        family_size = rng.rounded_gauss(AVERAGE_FAMILY_SIZE,1.5);   //a family as a mean
        if (family_size < 1) family_size = 1;
        Position try_pos{rng.uniform(lw_x, up_x), rng.uniform(lw_y, up_y)};
        ...
        for (unsigned int i = 0; i < generated_homes.size() ;++i)
        {
            ...
            {
                try_pos = {rng.uniform(lw_x, up_x), rng.uniform(lw_y, up_y)};  //try a new one
                i = 0;
