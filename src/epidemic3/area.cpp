...
namespace SMOOTH{
    ...
    //constructor
    Area::Area (double side,double transmission_range)
          :sd{side},R{transmission_range}
    {
        assert(sd > 0 && R > 0);
    }
   ...
   void Area::partition_in_clusters()
   {
    ...
    void Area::partition_in_groups(int label)
    {
        ...
        for (auto& group : Clusters[label].Groups )
        {
            ...
            std::cout << "Group "<<j<<" -->Waypoint["<<already_setted_waypoints<<"]  Size == "<<group.size()<<std::endl;;
    ...
    Location Area::first_group_step(int label)
    {
        ...
        {         //if 1 <= label <= C-1
            ...
            while(!end_loop)
            {
                ...
                for (int curr_cluster_index = 0; curr_cluster_index < check_up_to; ++curr_cluster_index) //loop over Clusters
                {
                      if (change_wpt) break;
                      for (auto &current_group : Clusters[curr_cluster_index].Groups) //loop over Groups
                      {
                          if (change_wpt) break;
                          int it_index = 0;
                          for (auto it = current_group.group_ptr; it_index < current_group.size(); it += it_index) //loop over group Waypoints
                          {
...
//                std::cout <<"location found in "<<i<<" loops\n";
                assert(try_waypoint.X() >0.0 && try_waypoint.Y() >0.0 && try_waypoint.X()<=sd && try_waypoint.Y()<=sd);
                return try_waypoint;
            }
            ++i;
        } //end while
