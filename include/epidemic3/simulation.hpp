...
class Simulation{
  ...
  private:
    ...
    void plot_waypoints();
    void fill_path_from_cluster(Person& person);   //select next visiting locations from the person cluster
    void fill_path(Person& person);                //select next visiting locations from green clusters
    void update_target(Person* person);            //update target through LATP algorithm
