...
class World
{
  private:
    ...
    Random& wrld_eng;
  public:
    World(double Side_length, int number_of_clusters, int number_of_location, int S, int E, int I, int R,Random& simulation_engine);
//    World();
    //unused functions
...
//    Cluster* get_cluster(int index);         // return a pointer to index cluster
    ...
    void generate_path(int to_visit, const std::vector<double>& weights, std::vector<Location*>& path,Random& simulation_engine);
};
