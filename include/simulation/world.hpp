...
class World
{
  ...
  public:
    ...
    void generate_path(int to_visit, const std::vector<double>& weights, std::vector<Location*>& path,
                       Random& rng); // generate path from selecting from all green clusters
};

std::vector<Location*> generate_path(
    std::vector<Location*> list, double mean,
    double dev); // generate a random number of pointer to Location from the provided one
